...
- **Alur Utama:** dari detail merchant (`/merchants/[id]`) atau driver (`/ride/drivers/[id]`), kartu **Saldo Dompet** menampilkan tombol **Pindah Saldo** → `/topup/transfer?userId=…&to=…&returnTo=…`. Halaman itu menampilkan saldo sumber & tujuan, meminta nominal, lalu mengonfirmasi dengan angka **sebelum → sesudah** sebelum memanggil `POST /admin/wallet/transfer`. Dibuka tanpa `userId` (tombol di `/topup`), halaman yang sama menjadi **dua langkah**: pilih pemilik akun dulu → halaman memuat saldonya → baru nominal diminta.
...
- **Guard saldo (berlapis):** nominal tidak pernah diminta sebelum saldo sumber terbaca — bila endpoint saldo gagal, halaman menolak melanjutkan alih-alih menampilkan nol. Di klien, nominal melebihi saldo (atau di luar 10rb–100jt) memunculkan pesan inline dan menonaktifkan tombol; input `max` juga dibatasi ke saldo tersedia. Guard yang mengikat tetap di backend — saldo bisa berubah antara halaman dirender dan tombol ditekan — dan `INSUFFICIENT_BALANCE` ditampilkan sebagai pesan di form, bukan halaman error.
- **Acceptance:** *Given* admin membuka detail merchant ber-`userId`, *When* memindahkan Rp20.000, *Then* kartu saldo dan `/payments` menampilkan hasilnya (debit di USER, credit di MERCHANT) tanpa mengubah total kredit/debit ringkasan. *Given* saldo pengguna Rp10.000, *When* admin mengetik Rp50.000, *Then* muncul "Melebihi saldo pengguna" dan tombol nonaktif. *Given* saldo berkurang setelah halaman dirender, *When* transfer dikirim, *Then* form menampilkan pesan saldo tidak mencukupi dan tidak ada redirect. *Given* OPERATOR, *When* memanggil action-nya, *Then* `FORBIDDEN` dan API tidak pernah dipanggil.
